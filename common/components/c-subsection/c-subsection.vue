<template>
	<view class="c-subsection" :style="[subsectionStyle]">
		<view class="c-subsection__item c-ellipsis" :style="[itemStyle(index)]" @tap="click(index)" :class="[noBorderRight(index), 'c-subsection__' + index]"
		 v-for="(item, index) in listInfo" :key="index">
			<view :style="[textStyle(index)]" class="c-item__text c-ellipsis">{{ item.name }}</view>
		</view>
		<view class="c-subsection__bg" :style="[itemBarStyle]"></view>
	</view>
</template>
...
<script>
	 ...
	 * @example <c-subsection active-color="#ff9900"></c-subsection>
	 */
	export default {
		name: "c-subsection",
		props: {
		...
		computed: {
			...
			noBorderRight() {
				return index => {
					...
					// 显示整个组件的左右边圆角
					if (index == 0) classs += ' c-subsection__first';
					if (index == this.list.length - 1) classs += ' c-subsection__last';
					return classs;
				};
			},
			// 文字的样式
		...
		methods: {
			...
			// 获取各个tab的节点信息
			async getTabsInfo() {
				let res = await this.$c.getRect.call(this, '.c-subsection__item')
				// 将分段器每个item的宽度，放入listInfo数组
				res.forEach((o, i) => {
					this.listInfo[i].width = o.width;
				})
				// 初始化滑块的宽度
				if (this.mode == 'subsection') {
