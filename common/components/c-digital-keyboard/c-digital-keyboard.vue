<template>
	<view class="digital-key">
		<view class="digital-key-hd" v-if="showDown_">
			<view class="digital-key__icon" @click="$emit('close');"></view>
			<slot></slot>
		</view>
		<view class="digital-key-bd" @click="emitKeyNumber">
			<view class="digital-key__num" v-for="item in digitalList" :data-num="item" :key="item" :class="{'digital-key__num--no':(item === '' || item === '-1')}">{{item}}</view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		computed: {
			...
			showDown_() {
				return String(this.showDown) === 'false' ? false : true;
			}
		},
		watch: {
			activeIndex: {
