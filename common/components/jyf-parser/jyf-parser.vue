...
export default {
    ...
    computed: {
        style_() {
            return this.showAm + (this.selectable ? ';user-select:text;-webkit-user-select:text' : '')
        }
    },
    watch: {
        html(html) {
    ...
    created() {
        ...
        this.imgList.setItem = function (i, src) {
            ...
            if (src.includes('data:image')) {
                var filePath; var info = src.match(/data:image\/(\S+?);(\S+?),(.+)/)
                if (!info) return
                // #ifdef MP
    ...
    methods: {
        ...
        setContent(html, append) {
            ...
            // #ifdef H5 || MP-360
            if (!html) {
                if (this.rtf && !append) this.rtf.parentNode && this.rtf.parentNode.removeChild(this.rtf)
                return
            }
            ...
            var fill = target => {
                ...
                if (this.domain && src) {
                    if (src[0] == '/') {
                        if (src[1] == '/') { target.src = (this.domain.includes('://') ? this.domain.split('://')[0] : '') + ':' + src } else target.src = this.domain + src
            ...
            for (let i = 0, j = 0, img; img = imgs[i]; i++) {
                if (parseInt(img.style.width || img.getAttribute('width')) > windowWidth) { img.style.height = 'auto' }
                fill(img)
        ...
        getText(ns = this.nodes) {
            ...
            for (var i = 0, n; n = ns[i++];) {
                if (n.type == 'text') {
                    txt += n.text.replace(/&nbsp;/g, '\u00A0').replace(/&lt;/g, '<').replace(/&gt;/g, '>')
                        .replace(/&amp;/g, '&')
        ...
        navigateTo(obj) {
            ...
            // #ifdef APP-PLUS-NVUE
            if (!obj.id) { weexDom.scrollToElement(this.$refs.web) } else {
            ...
            switch (d.action) {
                ...
                case 'linkpress':
                    ...
                    if (jump && href) {
                        ...
                        } else if (href.includes('://')) { plus.runtime.openWeb(href) } else {
                            uni.navigateTo({
                                url: href
                            })
                        }
