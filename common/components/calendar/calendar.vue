<template>
    ...
    >
        <view
            ...
            :class="{'tui-popup-show': isShow_}"
        >
            <view
                class="tui-calendar-header"
                :class="{ 'tui-calendar-radius': radius }"
            >
                <view>日期选择</view>
                <view
                    class="tui-iconfont tui-font-close"
            ...
            <view
                class="tui-date-content"
                :class="{ 'tui-flex-start': isFixed && fixedHeight }"
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        <text
                            v-if="getStatusData(4, index)"
                            class="tui-iconfont tui-font-check"
                        ></text>
                    </view>
                    <view
...
export default {
    ...
    computed: {
        ...mapState({
            shopConfig_: (state) => state.config.shopConfig ? state.config.shopConfig : { color: { theme: '#5677fc' } }
        }),
        dataChange() {
    ...
    methods: {
        getColor(index, type) {
            const { color: { theme } } = this.shopConfig_
            let colorBox = type === 1 ? '' : this.color
            const day = index + 1
            ...
            if ((this.isActiveCurrent && this.activeDate === date) || this.startDate === date || this.endDate === date) {
                colorBox = type === 1 ? this.activeBgColor || theme : this.activeColor
            } else if (this.endDate && timestamp > new Date(start).getTime() && timestamp < new Date(end).getTime()) {
                colorBox = type === 1 ? this.rangeBgColor : this.rangeColor || theme
            }
            return colorBox
        },
        // 获取状态数据
