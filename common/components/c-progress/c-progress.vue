<template>
    <view class="c-progress c-progress-default" :class="setStatusClass">
        <block v-if="type == 'circle' || type == 'dashboard'">
            <view class="c-progress c-progress-default" :class="setStatusClass">
                <view class="c-progress-inner" :style="setCircleStyle">
                    <!-- 绘制圈 start -->
                    <!-- #ifdef H5 -->
                    <svg viewBox="0 0 100 100" class="c-progress-circle">
                        <path :d="setCirclePath" stroke="#f3f3f3" :stroke-linecap="strokeShape" :stroke-width="strokeWidth"
                            fill-opacity="0" class="c-progress-circle-trail" :style="setCircleTrailStyle"></path>
                        <path :d="setCirclePath" :stroke-linecap="strokeShape" :stroke-width="strokeWidth" fill-opacity="0"
                            class="c-progress-circle-path" :style="setCirclePathStyle"></path>
                    </svg>
                    <!-- #endif -->
                    <!-- #ifndef H5 -->
                    ...
                    <!-- 状态文本 start -->
                    <block v-if="showInfo">
                        <text class="c-progress-text" :title="setFormat" :style="[ textStyle ]">
                            <block v-if="status != 'success' && status != 'exception' && setProgress < 100">{{setFormat}}</block>
                            <block v-if="showIcon">
                                <!-- #ifdef H5 -->
                                <svg v-if="status == 'exception'" viewBox="64 64 896 896" data-icon="close" width="1em"
                                    height="1em" fill="currentColor" aria-hidden="true">
                                    <path d="M563.8 512l262.5-312.9c4.4-5.2.7-13.1-6.1-13.1h-79.8c-4.7 0-9.2 2.1-12.3 5.7L511.6 449.8 295.1 191.7c-3-3.6-7.5-5.7-12.3-5.7H203c-6.8 0-10.5 7.9-6.1 13.1L459.4 512 196.9 824.9A7.95 7.95 0 0 0 203 838h79.8c4.7 0 9.2-2.1 12.3-5.7l216.5-258.1 216.5 258.1c3 3.6 7.5 5.7 12.3 5.7h79.8c6.8 0 10.5-7.9 6.1-13.1L563.8 512z"></path>
                                </svg>
                                <svg v-if="status == 'success' || setProgress == 100" viewBox="64 64 896 896" data-icon="check"
                                    width="1em" height="1em" fill="currentColor" aria-hidden="true" :style="{'color': strokeColor ? strokeColor : ''}">
        ...
        <block v-if="type == 'line'">
            <!-- 进度条 start -->
            <view class="c-progress-outer">
                <view class="c-progress-inner" :style="{'border-radius': strokeShape == 'square' ? 0 : '100px', 'background-color': strokeBg}">
                    <view class="c-progress-bg" :style="setLineStyle"></view>
                    <view v-if="successPercent" class="c-progress-success-bg" :style="setLineSuccessStyle"></view>
                </view>
            </view>
            <!-- 进度条 end -->
            <!-- 进度条是否显示信息 start -->
            <block v-if="showInfo">
                <text class="c-progress-text" :title="setFormat" :style="[ textStyle ]">
                    ...
                    <block v-if="showIcon">
                        <!-- #ifdef H5 -->
                        <svg v-if="status == 'exception'" viewBox="64 64 896 896" data-icon="close-circle" width="1em"
                            height="1em" fill="currentColor" aria-hidden="true">
                            <path d="M512 64C264.6 64 64 264.6 64 512s200.6 448 448 448 448-200.6 448-448S759.4 64 512 64zm165.4 618.2l-66-.3L512 563.4l-99.3 118.4-66.1.3c-4.4 0-8-3.5-8-8 0-1.9.7-3.7 1.9-5.2l130.1-155L340.5 359a8.32 8.32 0 0 1-1.9-5.2c0-4.4 3.6-8 8-8l66.1.3L512 464.6l99.3-118.4 66-.3c4.4 0 8 3.5 8 8 0 1.9-.7 3.7-1.9 5.2L553.5 514l130 155c1.2 1.5 1.9 3.3 1.9 5.2 0 4.4-3.6 8-8 8z"></path>
                        </svg>
                        <svg v-if="status == 'success' || setProgress == 100" viewBox="64 64 896 896" data-icon="check-circle"
                            width="1em" height="1em" fill="currentColor" aria-hidden="true" :style="{'color': strokeColor ? strokeColor : ''}">
...
<script>
     ...
     * @example <c-progress :percent="30"></c-progress>
     */
    export default {
        name: 'c-progress',
        ...
        computed: {
            ...
            setCircle() {
                ...
                let svgToBase =
                    `data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100' class='c-progress-circle'%3E%3Cpath d='${this.setCirclePath}' stroke='%23f3f3f3' stroke-linecap='${this.strokeShape}' stroke-width='${this.strokeWidth}' fill-opacity='0' class='c-progress-circle-trail' style='stroke-dasharray: ${len - (gapDeg||0)}px, ${len}px;stroke-dashoffset: -${(gapDeg||0) / 2}px;transition: stroke-dashoffset 0.3s ease 0s, stroke-dasharray 0.3s ease 0s, stroke 0.3s;'%3E%3C/path%3E%3Cpath  d='${this.setCirclePath}' stroke-linecap='${this.strokeShape}' stroke-width='${this.strokeWidth}' fill-opacity='0' class='c-progress-circle-path' style='stroke: ${escape(currentColor)};stroke-dasharray: ${(this.setProgress / 100) * (len - (gapDeg||0))}px, ${len}px;stroke-dashoffset: -${(gapDeg||0) / 2}px;transition: stroke-dashoffset 0.3s ease 0s, stroke-dasharray 0.3s ease 0s, stroke 0.3s, stroke-width 0.06s ease 0.3s;'%3E%3C/path%3E%3C/svg%3E`
            ...
            setStatusClass() {
                ...
                // 异常进度
                if (this.status == 'exception') {
                    statusClass.push('c-progress-status-exception')
                }
                // 完成进度
                if (this.status == 'success' || this.setProgress >= 100) {
                    statusClass.push('c-progress-status-success')
                }
                // 活动进度条
                if (this.status == 'active') {
                    statusClass.push('c-progress-status-active')
                }
                // 是否显示信息
                if (this.showInfo) {
                    statusClass.push('c-progress-show-info')
                }
                // 进度条类型
                if (this.type === 'line') {
                    statusClass.push('c-progress-line')
                }
                // 进度圈、仪表盘类型
                if (this.type === 'circle' || this.type === 'dashboard') {
                    statusClass.push('c-progress-circle')
                }
                statusClass.push('c-progress-status-normal')
                return statusClass;
            }
        }
    }
...
<style>
    ...
    .c-progress-show-info .c-progress-outer {
        flex: 1;
    }
    ...
    .c-progress-circle-trail {
        stroke: #f5f5f5;
    }

    .c-progress-circle-path {
        stroke: #1890ff;
        animation: appear 0.3s;
    }

    .c-progress-success-bg,
    .c-progress-bg {
    ...
    .c-progress-status-active .c-progress-bg:before {
        content: "";
        opacity: 0;
        position: absolute;
        top: 0;
        ...
        background: #fff;
        border-radius: 20upx;
        -webkit-animation: c-progress-active 2.4s cubic-bezier(0.23, 1, 0.32, 1) infinite;
        animation: c-progress-active 2.4s cubic-bezier(0.23, 1, 0.32, 1) infinite;
    }

    .c-progress-status-exception .c-progress-bg {
        background-color: #f5222d;
    }

    .c-progress-status-exception .c-progress-text {
        color: #f5222d;
    }

    .c-progress-status-exception .c-progress-circle-path {
        stroke: #f5222d;
    }

    .c-progress-status-success .c-progress-bg {
        background-color: #52c41a;
    }

    .c-progress-status-success .c-progress-text {
        color: #52c41a;
    }

    .c-progress-status-success .c-progress-circle-path {
        stroke: #52c41a;
    }

    .c-progress-circle .c-progress-inner {
        position: relative;
        line-height: 1;
        background-color: transparent;
    }

    .c-progress-circle .c-progress-text {
        display: block;
        position: absolute;
        width: 100%;
        text-align: center;
    ...
    .c-progress-circle .c-progress-status-exception .c-progress-text {
        color: #f5222d;
    }

    .c-progress-circle .c-progress-status-success .c-progress-text {
        color: #52c41a;
    }

    @keyframes c-progress-active {
        0% {
            opacity: 0.1;
            width: 0;
        }
